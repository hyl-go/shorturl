...
var (
	...
	cacheShorturlShortUrlMapLurlPrefix = "cache:shorturl:shortUrlMap:lurl:"
...
type (
	...
	ShortUrlMap struct {
		...
		Category        sql.NullString `db:"category"`         // 链接分类
		SafetyStatus    uint64         `db:"safety_status"`    // 安全状态
		PageTitle       sql.NullString `db:"page_title"`       // 页面标题
		PageDescription sql.NullString `db:"page_description"` // 页面描述
		AiSuggestions   []byte         `db:"ai_suggestions"`   // AI 建议名称
	}
)
...
func (m *defaultShortUrlMapModel) FindOneByLurl(ctx context.Context, lurl sql.NullString) (*ShortUrlMap, error) {
	shorturlShortUrlMapLurlKey := fmt.Sprintf("%s%v", cacheShorturlShortUrlMapLurlPrefix, lurl)
	var resp ShortUrlMap
	err := m.QueryRowIndexCtx(ctx, &resp, shorturlShortUrlMapLurlKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
		query := fmt.Sprintf("select %s from %s where `lurl` = ? limit 1", shortUrlMapRows, m.table)
		if err := conn.QueryRowCtx(ctx, &resp, query, lurl); err != nil {
			return nil, err
		}
		return resp.Id, nil
	}, m.queryPrimary)
